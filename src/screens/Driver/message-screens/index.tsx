...
const DriverMessagesScreen = (props: any) => {
  // Variables
  ...
  console.log('Chats ===>', JSON.stringify(Chatss, null, 2));
  // Effects
  useFocusEffect(
    React.useCallback(() => {
      setTimeout(() => {
    ...
    return (
      <FlatList
        ...
        renderItem={({item, index}) => (
          <View
            onTouchEnd={() => {
              navigation.navigate('DriverChatScreen', {data: item});
            }}
            key={item}
            ...
            <View
              ...
              <Image
                style={styles.image3}
                resizeMode="cover"
                source={{uri: item?.company?.profilePicture}}
              />
              <View style={{marginLeft: 10 * widthRef}}>
                <Text style={styles.text6}>{item?.company?.name}</Text>
