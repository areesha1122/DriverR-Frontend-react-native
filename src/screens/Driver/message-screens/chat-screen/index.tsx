...
const DriverChatScreen = (props: any) => {
  // Variables
  ...
  useEffect(() => {
    ...
    const socketLeave = () => {
      sockett.emit('leave-room', {
        sender: route.params.data.company._id,
  ...
  const socketCall = async () => {
    ...
    sockett.on('driver-company-chat', (data: any) => {
      ...
      console.log('message from backend driver', data);
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.searchView}>
        ...
        <View
          ...
          <Image
            style={styles.image3}
            resizeMode="cover"
            source={{uri: route.params.data.company.profilePicture}}
          />
          <View style={{marginLeft: 10 * widthRef}}>
            <Text language={language} style={styles.text6}>
              {route.params.data.company.name}
            </Text>
          ...
          {chat.status == 'BLOCKED' && chat.blockedBy != profile._id ? null : (
            <Icon
              ...
              style={{
                position: 'absolute',
                right: 10 * widthRef,
                bottom: Platform.OS == 'ios' ? 12 * heightRef : 10 * heightRef,
                zIndex: 1,
              }}
            />
          )}
      ...
      {chat.status == 'BLOCKED' ? (
        <View
          ...
          <Text style={{color: colors.black, marginBottom: 20 * heightRef}}>
            You cannot message to this chat
          </Text>
        </View>
      ) : (
        <View
              ...
              onPress={() => {
                ...
                if (file?.uri == undefined && message == '') {
                  ...
                  sockett.emit('driver-company-chat', {
                    ...
                    companyId: route.params.data.company._id,
                    sendBy: 'driver',
                    message: message,
                    attachment: '',
                  });
